...
<Stack>
	<Center>
		<Group>
            ...
            <Title order={3}>{currentLang === "en" ? "Played" : "Gespielt"}: {num.used} | {currentLang === "en" ? "Remaining" : "Übrig"}: {num.left}</Title>
		</Group>
	</Center>
	...
	<Center>
		<Card shadow="xl" withBorder={true}  style="width: 80vw">
			...
			<Container style="padding: 5rem">
				<Center>
                    <Stack>
                        <Title order={2}>{currentLang === "en" ? card.saying.en : card.saying.de}</Title>
                        <Title order={3}>{currentLang === "en" ? card.rule.en : card.rule.de}</Title>
                    </Stack>
				</Center>
			</Container>
			<Space h="xl"/>
