<script>
	import { Stack, Accordion, TextInput, Switch, Container, Grid, Center, Title, Button } from '@svelteuidev/core';
...
<Container size="xl">
    <Stack spacing="xl">
        <Grid>
            ...
            <Grid.Col span={4}>
            <Button on:click={addPlayer} disabled={name.length === 0} fullSize>
                {#if currentLang == "en"}
                Add Player
                {:else}
                Spieler Hinzufügen
