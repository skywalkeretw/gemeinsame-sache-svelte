<script>
    import { Badge, Accordion, Flex, Switch, Container, Grid, Center, Title, Button } from '@svelteuidev/core';
...
<Accordion.Item value="{player.id}">
    ...
    <Grid>
        <Grid.Col span={10}>
            <Flex gap="lg">
                ...
                <Badge size="xl" radius="sm">
                    {currentLang == "en" ? getGameByKey(game).en : getGameByKey(game).de}: {score}
                </Badge>
                {/each}
            </Flex>
        </Grid.Col>
        <Grid.Col span={2}>
            ...
            label={player.active ? (currentLang == "en" ? "Player Active" : "Spieler Aktiv") : (currentLang == "en" ? "Player Disabled" : "Spieler Deaktiviert") }
            />
        </Grid.Col>
    </Grid>
